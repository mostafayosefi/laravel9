...
<td> <a href="{{ route('user.domain.renew.show', $admin) }}"> @include('index.layouts.table.getstatus', [$admin ,'route' => ''  ,'type_name' => 'status_domain'   ]) </a> </td>



        <td>
            @php $admin['title'] = $admin->domain->domain;  @endphp
            @include('admin.layouts.table.modal', [$admin ,'route' => route('user.domain.renew.destroy', $admin) , 'myname' => 'حذف سفارش تمدید دامنه '.$admin->domain->domain ])
        </td>
