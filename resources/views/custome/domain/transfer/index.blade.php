...
<td> <a href="{{ route('user.domain.transfer.show', $admin) }}"> @include('index.layouts.table.getstatus', [$admin ,'route' => ''  ,'type_name' => 'status_domain'   ]) </a> </td>



        <td>
            @php $admin['title'] = $admin->domain;  @endphp
            @include('admin.layouts.table.modal', [$admin ,'route' => route('user.domain.transfer.destroy', $admin) , 'myname' => 'حذف سفارش انتقال دامنه '.$admin->domain ])
        </td>
